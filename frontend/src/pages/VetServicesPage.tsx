...
export default function VetServicesPage() {
  ...
  useEffect(() => {
    const fetchVets = async () => {
      ...
      if (rolesError) {
        console.error("Error fetching veterinarian roles:", rolesError);
        setLoading(false);
        return;
      }
      ...
      const { data: profiles, error: profilesError } = await supabase
        .from("profiles")
        .select("user_id, full_name, organization_name, address")
        .in("user_id", vetIds)
        .eq("verification_status", "approved");

      if (profilesError) {
        console.error("Error fetching veterinarian profiles:", profilesError);
      }
  ...
  const fetchSlots = async (vetId: string) => {
    ...
    if (error) {
      ...
      toast({ title: "Unable to load slots", variant: "destructive" });
    }
  ...
  const handleBookAppointment = async () => {
    ...
    if (role === "pet_owner" && userPets.length > 0 && !selectedPetId) {
      toast({ title: "Please select your pet", variant: "destructive" });
      return;
    }
          ...
          <div className="space-y-4">
            {userPets.length > 0 && (
              <div className="space-y-2">
                <Label>Select Your Pet {role === "pet_owner" ? "*" : "(optional)"}</Label>
                <Select value={selectedPetId} onValueChange={setSelectedPetId}>
                  <SelectTrigger><SelectValue placeholder={role === "pet_owner" ? "Choose your pet" : "Choose a pet (optional)"} /></SelectTrigger>
                  <SelectContent>
                    {userPets.map(p => (
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <Button onClick={handleBookAppointment} disabled={!selectedSlot || (role === "pet_owner" && userPets.length > 0 && !selectedPetId)}>
                Request Appointment
              </Button>
            </div>
          </div>

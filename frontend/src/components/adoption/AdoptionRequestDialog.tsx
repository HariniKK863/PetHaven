...
export function AdoptionRequestDialog({ open, onOpenChange, pet, userId, onSuccess }: AdoptionRequestDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await supabase.from("adoption_requests").insert({
      ...
      pet_location: pet.location || null,
      pet_shelter_name: pet.shelter_name || null,
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="phone" type="tel" value={form.phone} onChange={(e) => handleChange("phone", normalizeIndianPhoneInput(e.target.value))} placeholder="98765 43210" />
          </div>
          <div className="space-y-2">

...
export function AdminDashboard() {
  ...
  const fetchPendingVerifications = async () => {
    ...
    if (rolesError) {
      ...
      toast({
        title: "Error",
        description: "Failed to load account roles for verification requests.",
        variant: "destructive",
      });
      setLoading(false);
      return;
  ...
  const handleViewDocument = async (documentRef: string) => {
    try {
      await openVerificationDocument(documentRef);
    } catch (error) {
      console.error("Error opening verification document:", error);
      toast({
        title: "Unable to open document",
        description: "Failed to access the uploaded verification document.",
        variant: "destructive",
      });
    }
  };
